...
#define BEGIN_CMD()  vTaskSuspendAll(); TX_8(0XEE)            //帧头
#define END_CMD() 	 TX_32(0XFFFCFFFF); xTaskResumeAll()       //帧尾
...
void ReadTextValue(uint16 screen_id, uint16 control_id)
{
    BEGIN_CMD();
    TX_8(0xB1);
    TX_8(0x11);
...
*  \brief     读取屏幕RTC时间
*/
void SetRTC(uint8 year, uint8 month, uint8 week, uint8 day, uint8 hour, uint8 minute, uint8 second)
{
    BEGIN_CMD();
    TX_8(0x81);
    TX_8(second);
